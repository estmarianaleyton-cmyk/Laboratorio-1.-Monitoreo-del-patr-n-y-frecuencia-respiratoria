...
El uso de varios sensores podría brindar varias ventajas, primero permitiría registrar simultáneamente los movimientos de diferentes regiones del cuerpo asociados a la respiración, proporcionando una representación más completa del proceso respiratorio. Además, si uno de los sensores pierde contacto o presenta ruido debido al movimiento del paciente, los demás podrían compensar esta pérdida de información, aumentando la confiabilidad de las mediciones.

Sin embargo, esta opción presenta unas desventajas. Incrementa la complejidad del sistema, ya que requiere más etapas de acondicionamiento de señal, mayor cantidad de canales de adquisición y todo el proceso de integrar la información proveniente de todos los sensores. Asimismo, aumenta el costo del sistema y además puede afectar la comodidad del usuario, debido a la necesidad de colocar varios dispositivos sobre el cuerpo.

En el contexto de esta práctica, el uso de un único sensor FSR402 fue suficiente para identificar el patrón respiratorio y estimar la frecuencia respiratoria en ambas condiciones. No obstante, la mayor variabilidad observada durante la lectura sugiere que un sistema con más sensores podría mejorar la calidad del registro al captar de forma más completa los cambios en la mecánica respiratoria asociados al habla, sobre todo en situaciones donde el movimiento del tórax no refleja todo el proceso respiratorio.

# **Conclusiones**

-Fue posible implementar un sistema de adquisición basado en el uso de un sensor FSR402 que logró registrar adecuadamente el patrón respiratorio permitiendo estimar la frecuencia respiratoria del participante en las dos condiciones evaluadas. El procesamiento de la señal por medio de la eliminación de la componente DC, filtrado Butterworth y detección de picos permitió identificar los ciclos respiratorios y obtener resultados aceptables. 

-Si bien la frecuencia respiratoria estimada por conteo de picos fue igual entre condiciones (12 rpm), el patrón respiratorio evidenció cambios significativos. Durante la lectura del párrafo se pudo observar cómo los ciclos respiratorios presentaban diferentes amplitudes y duraciones. Estos resultados confirman que el hecho de hablar altera la periodicidad normal de la respiración ya que se debe establecer una relación entre el mecanismo respiratorio y la producción vocal. 

-En cuanto al análisis en el dominio de la frecuencia, este presentó una pequeña discrepancia en comparación con el método basado en detección de picos en la condición de verbalización, lo cual evidencia cómo la menor periodicidad de la señal respiratoria puede afectar la estimación de la frecuencia respiratoria mediante FFT. Por ende, fue posible concluir la importancia de utilizar distintas estrategias de análisis para obtener una mayor caracterización de la señal respiratoria. 

-Por último, se obtuvieron resultados similares a los reportados en la literatura y pudimos evidenciar que el sensor FSR402 es una opción válida para la adquisición de señales respiratorias no invasivas. Además, se pudo comprender cómo actividades como la verbalización alteran el patrón respiratorio sin afectar de manera significativa la frecuencia respiratoria promedio. 
